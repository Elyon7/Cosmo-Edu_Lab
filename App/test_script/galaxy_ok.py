...
def fit_galaxy_parameters(r, Vobs, errV, Vgas, Vdisk, Vbul):
    ...
    for log_rs in log_rs_grid:
        ...
        for log_rhos in log_rhos_grid:
            ...
            # Calcolo velocità barionica sdoppiato: Disco 0.5, Bulge 0.7
    ...
    print(f"Parametri Profilo     : (rho_s={rho_s_fin:.2e}, r_s={r_s_fin:.2f}, Y_D={y_disk_fin:.2f}, Y_B={y_bul_fin:.2f})")
    print(f"Ha toccato un bordo?  : {hit_bound_str}")
    print(f"----------------------------------\n")
    
    if res.success and res.fun < best_chi2:
        return res.fun, rho_s_fin, r_s_fin, y_disk_fin, y_bul_fin, M200_fin, hit_bound_str  
    
    return best_chi2, rho_s_fin, r_s_fin, y_disk_fin, y_bul_fin, M200_fin, hit_bound_str

def run_pipeline():
    input_folder = "Rotmod_LTG"
    output_folder = "barke_rs_rhos_upsilon_split"
    export_file = "barke_split_results.csv"
    ...
    print("-" * 118)
    print(f"{'Galassia':<15} | {'Q':<3} | {'\u03C7\u00B2 Min':<8} | {'\u03C1_s':<9} | {'r_s':<6} | {'\u03A5_D/B':<7} | {'M_bar_edg':<9} | {'M_DM_edg':<9} | {'M_tot_edg':<9} | {'% DM':<6}")
    print(f"{'':<15} | {'':<3} | {'':<8} | {'(M_s/kpc\u00B3)':<9} | {'(kpc)':<6} | {'(M/L)':<7} | {'(M_sun)':<9} | {'(M_sun)':<9} | {'(M_sun)':<9} | {'(bordo)':<6}")
    print("-" * 118)
    ...
    for file in sorted(files):
        ...
        try:
            ...
            chi2_min, rho_s, r_s, y_disk, y_bul, m200, hit_bound_str = fit_galaxy_parameters(r, Vobs, errV, Vgas, Vdisk, Vbul)
            if hit_bound_str == "SÌ":
                print(f"{gal_name:<15} | SCARTATA: Parametro (rs o rhos) ha raggiunto il bordo")
                continue
                
            # Calcolo velocità barionica sdoppiato
            V_bar_sq = Vgas * np.abs(Vgas) + 0.5 * (Vdisk * np.abs(Vdisk)) + 0.7 * (Vbul * np.abs(Vbul))
            ...
            ax1.plot(r, V_bar, '-', color='red', linewidth=2, label=f'Baryonic (D=0.5, B=0.7)')
    ...
    print("Salvataggio 'galaxy_best_parameters.csv' completato!")
    ...
    print(r"\multicolumn{6}{c}%") 
    print(r"{{\bfseries \tablename\ \thetable{} -- continued from previous page}} \\")
    print(r"\hline")
    print(r"Galaxy & $r_s$ [kpc] & $\rho_s$ [$M_\odot/\text{kpc}^3$] & $\chi^2_{\text{min}}$ & $M_{\text{tot}}$ [$M_\odot$] & $\% \text{DM}/M_{\text{tot}}$ \\")
    print(r"\hline")
    print(r"\endhead")
    print(r"\hline \multicolumn{6}{r}{{Continued on next page}} \\") 
    print(r"\endfoot")
    print(r"\hline")
    print(r"\endlastfoot")
